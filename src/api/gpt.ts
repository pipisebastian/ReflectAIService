...
export const getAIResponse = async ({ prompt }: { prompt: string }): Promise<AIResponse> => {
  const messages = [
    {
      role: "system",
      content: `## INFO ##
    ...
    },
    {
      ...
      content: `1. [title] : Think of the diary title after understanding the [events] separated by """ at the bottom.
        ...
        Translate into Korean and Use the output in the following JSON format. 각 내용은 한국어로 번역해서 보내줘:
        { 
            "title": here is [title],
  ...
  const jsonData = JSON.parse(responseData.choices[0].message.content) as AIResponse;
